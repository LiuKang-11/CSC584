...
static const float MAX_SPEED       = 25.f;      
static const float LOOK_AHEAD_DIST = 20.f;     
static const float AGENT_RADIUS = 12.f;   
...
/*────────────────────  BoidSprite ────────────────────*/
/* —— SteeringBehaviour / Position / Velocity / Rotation  ——*/
...
class Position: public SteeringBehaviour{/*…保持不變…*/ 
public:
...
class Velocity: public SteeringBehaviour{/*…保持不變…*/ 
public:
...
class Rotation: public SteeringBehaviour{/*…保持不變…*/ 
public:
...
public:
    float wanderMax = 25.f;      // ★ 新增：wander 時的極速
...
private:
    ...
    void buildDoorPath(int from, int to){
        auto seq = graph_.a_star(from,to);
        if(seq.empty()) return;
        for(size_t i=0;i+1<seq.size();++i){
            ...
            for(auto &eb : rooms_[B].room_entry){
                ...
                // Door is the segment [ea, eb]
                sf::Vector2f dir = normalize(eb - ea);
                float length = vecDistance(ea, eb);
                int samples = 5;
                for(int s=1; s<samples; ++s){
                    // 在门口内沿门缝方向均匀采样
                    float t = (s + 0.5f) / float(samples);
                    sf::Vector2f pt = ea + dir * (t * length);
                    pt += dir * (AGENT_RADIUS + 1.f);
                    path_.push_back(pt);
                }
                goto nextDoor;
                }
...
enum class Action { Seek=0, ChangeRoom=1, Wander=2 };
static const char* ActionName[] = { "Seek", "ChangeRoom", "Wander" };

// policy for Seek，ChangeRoom，Wander
Action dt_policy(bool sameRoom, bool neighborRoom) {
    if (sameRoom)          return Action::Seek;
    else if (neighborRoom) return Action::ChangeRoom;
    else                   return Action::Wander;
}

/*────────────────────  main ────────────────────*/
int main(){
    srand((unsigned)time(nullptr));
    sf::RenderWindow window(sf::VideoMode(640,480),"HW4 Part3");
    ...
    // open CSV
    std::ofstream csv("DecisionTreeLearning.csv");
    csv << "SpriteRoom,MonsterRoom,SameRoom,NeighborRoom,Distance,State\n";
    ...
    while(window.isOpen()){
        ...
        auto startNewWanderInRoom = [&](int roomId){
            ...
            do{
                ...
                vecDistance(nt, r.centre) < 35.f           
                || vecDistance(nt, player.get_position()) < 40.f );  
        ...
        /* collect state —— */
        int spriteRoom  = locateRoom(rooms, player.get_position());
        int monsterRoom = locateRoom(rooms, monster.pos());
        float dist = vecDistance(player.get_position(), monster.pos());
        bool sameRoom   = (spriteRoom == monsterRoom);
        bool neighborRoom = false;

        for(auto &edge : graph.adj[spriteRoom]){
            if(edge.first == monsterRoom){
                neighborRoom = true;
                break;
            }
        }
        ...
        /* DT action —— */
        Action act = dt_policy(sameRoom, neighborRoom);
